...
Remember from **discovr_08** that to get a summary of an existing model like [pupluv_lm]{.alt} that uses heteroscedasticity-consistent standard errors (i.e. robust significance tests and confidence intervals), we put the model into `model_parameters()` and set [vcov = TRUE, vcov.type = "HC4"]{.alt}. Try this in the code box:
...
```{r pupp_hc4-solution}
parameters::model_parameters(pupluv_lm, vcov = TRUE, vcov.type = "HC4") |> 
  knitr::kable(digits = 3)
```

```{r, echo = F}
hc_par <- parameters::model_parameters(pup_lm, vcov = TRUE, vcov.type = "HC4")
```
...
```{r cure_hc4-solution}
parameters::model_parameters(cure_lm, vcov = TRUE, vcov.type = "HC4") |> 
  knitr::kable(digits= 3)
```
