...
* `r robot(height = 2)` **bend-R** is our coding robot. She will help you to try out bits of R by writing the code for you.
...
* The tutorials are self-contained (you practice code in code boxes). However, so you get practice at working in RStudio I strongly recommend that you create an R markdown file within an Rstudio project and practice everything you do in the tutorial in the R markdown file, make notes on things that confused you or that you want to remember, and save it. Within this markdown file you will need to load the relevant packages and data. 
...
Set up an RStudio project in the way that [I recommend in this tutorial](http://milton-the-cat.rocks/learnr/r/r_getting_started/#section-working-in-rstudio), and save the data files to the folder within your project called `data`. Place this code in the first code chunk in your R Markdown document:
...
###	`r bmu2()` Extracting model information with `summary()` and `broom` 
...
The `lm()` function does not produce standardized betas but you can get them using the `model_parameters()` function from the **parameters** package. Once you have loaded this package, you simply place your model name into it, for example:
...
It is possible to print the standardized betas within the main output itself, but I don’t advise it because it can create conflicts with other things in R.
...
## `r fontawesome::fa("fas fa-bug", fill = "orange")` Unguided Examples
