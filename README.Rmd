...
However, the tutorials within this package were fully updated summer 2025 so to get the most recent version install it from github. 
...
The most recent version relies on the latest development versions of the `easystats` packages, so to make sure everything goes smoothly execute

```{r, eval = F}
easystats::install_latest()
```

Once the latest `easystats` packages are on CRAN I will release the updated version there.
