...
withr::with_options(list(parameters_exponentiate = FALSE), {
  ...
  test_that("ci", {
    ...
    expect_equal(ci(m2)$CI_low, c(-0.47982, -1.85096, 0.76044), tolerance = 1e-3)
    expect_equal(
    ...
    expect_message(expect_null(ci(m2, component = "zi")), "no zero-inflation component")
  })
  ...
  test_that("se", {
    ...
    expect_equal(standard_error(m2)$SE, c(0.62127, 0.08128, 0.08915), tolerance = 1e-3)
    expect_equal(
  ...
  test_that("p_value", {
    expect_equal(p_value(m1)$p, c(0.00792, 0, 0, 0.55054, 0, 0.00181), tolerance = 1e-3)
    expect_equal(p_value(m1, component = "cond")$p, c(0.00792, 0, 0), tolerance = 1e-3)
    expect_equal(
    ...
    expect_equal(p_value(m2)$p, c(0.23497, 0, 0), tolerance = 1e-3)
    expect_equal(p_value(m2, component = "cond")$p, c(0.23497, 0, 0), tolerance = 1e-3)
  ...
  test_that("model_parameters.mixed-random total", {
    ...
    expect_named(
      params,
      c("Group", "Level", "Parameter", "Coefficient", "Component", "Effects")
    )
    expect_identical(
      as.vector(params$Parameter),
      c(
  ...
  test_that("model_parameters.mixed-ran_pars", {
    ...
    expect_identical(params$Parameter, c("SD (Intercept)", "SD (Intercept)"))
    expect_identical(params$Component, c("conditional", "zero_inflated"))
    expect_equal(params$Coefficient, c(0.9312, 1.17399), tolerance = 1e-2)
  })
  ...
  test_that("model_parameters, dispersion", {
    ...
    expect_equal(
      mp$Coefficient,
      c(58.25869, -0.87868, 47.01676, 36.99492),
      tolerance = 1e-2
    )
    expect_identical(
      mp$Parameter,
      c("wt", "wt:mpg", "SD (Intercept)", "SD (Observations)")
    )
    expect_identical(
      mp$Component,
      c("conditional", "conditional", "conditional", "conditional")
  ...
  test_that("model_parameters, no dispersion, glmmTMB", {
    ...
    expect_equal(
      mp1$Coefficient,
      unname(exp(unlist(glmmTMB::fixef(m_exp)$cond))),
      tolerance = 1e-3
    )
    expect_equal(
      mp2$Coefficient,
      unname(unlist(glmmTMB::fixef(m_exp)$cond)),
      tolerance = 1e-3
    )
  })
  ...
  test_that("print-model_parameters glmmTMB CI alignment", {
    ...
    model_pr <- tryCatch(
      {
        load(url(
          "https://github.com/d-morrison/parameters/raw/glmmTMB/data/pressure_durations.RData"
        ))
        glmmTMB::glmmTMB(
