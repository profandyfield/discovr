...
description: "Linear models involving continuous and categorical predictors. The first example looks at the case o moderation (non-paralell slopes models), whereas the second explores comparing means adjusted for other variables (a parallel slopes model or 'Analysis of Covariance (ANCOVA)'). The tutorial covers setting contrasts, fitting the models, evaluating effects using *F*-statistics based on Type III sums of squares and diagnostic plots, and interpretting the model using heteroscedasticity-consistent tests of parameters and *post hoc* tests."
...
puptreat_tib <- discovr::puppy_ancova
...
## `r user_visor()` Puppy love! [(B)]{.lbl}

The examples in this tutorial extend the example about puppy therapy from `discovr_11`. On the assumption that you can't have enough puppies in your life, here's another picture of my dog looking cute to help you to deal with the psychological trauma of this statistics tutorial.
...
The previous tutorial focussed on an example in which a researcher tested the efficacy of puppy therapy by exposing different groups of randomly-assigned people to (1) a no puppies control group; (2) 15 minutes of puppy therapy; and (3) 30 minutes of puppy contact. The dependent variable was a measure of happiness ranging from 0 (as unhappy as I can possibly imagine being) to 10 (as happy as I can possibly imagine being). We develop this example in two ways, in both cases we have a model in which there is both a categorical and continuous predictor.

1. Non-parallel slopes model: we look at a model in which we predict an interaction between the categorical and continuous predictor.
2. Parallel slopes model: we look at a model in which we *don't* predict an interaction between the categorical and continuous predictor. 

We first look at an example of a model containing both a categorical and continuous variable and fit a model following our usual 5-step process. We then move on to a special case of this kind of model, historically called 

The first model lends itself to the 5-step modelling process we've used in previous tutorials (Figure 5) whereas the second is a special case that in experimental research is used to adjust the outcome means across the categorical predictor by the continuous predictor. Historically, this approach equates to analysis of covariance (ANCOVA).
...
To begin with, imagine the researchers who conducted the puppy therapy study realized that a participants love of dogs could moderate whether puppy therapy affected happiness. They replicated the study on different participants, but included a self-report measure of love of puppies from 0 (I am a weird person who hates puppies, please be deeply suspicious of me) to 7 (puppies are the best thing ever, one day I might marry one).

The data are in [pupluv_tib]{.alt}, which contains the variables `id` (the participants id code), `dose` (1 = control, 2 = 15 minutes, 3 = 30 minutes), `happiness` (the persons happiness on a scale from 0-10), and `puppy_love` (the love of puppies from 0 to 7). They predict an interaction between `dose` and `puppy_love`.
...
Note that there are four variables: the participant's `id`, which is a character variable (note the `<chr>` under the name), the `dose` of puppy therapy, which is a factor (note the `<fct>` under the name), and the happiness score which is numeric and has the data type 'double' (note the `<dbl>` under the name) and their love of puppies (`puppy_love`), which also has the data type 'double'.
...
puppy_tib |> 
  group_by(dose) |> 
  describe_distribution(select = xxxx, ci = xxxx)
...
pupluv_tib |> 
  group_by(dose) |> 
  describe_distribution(select = happiness, ci = 0.95) |> 
  display()
```
...
Use what you already know to plot a violin plot of happiness scores in the three puppy therapy groups. Plot the raw data and display each group in a different colour.

```{r pup_violin, exercise = TRUE, exercise.lines = 10}

```

```{r pup_violin-solution}
...
Now let's look at the relationship between puppy love and happiness in the three therapy groups. Use what you already know to plot a scatterplot and linear model of the relationship between puppy love and happiness scores. Plot the three puppy therapy groups in a different colour.

```{r pup_scatter, exercise = TRUE, exercise.lines = 10}

```

```{r pup_scatter-solution}
...
It looks like puppy love has a positive relationship to happiness in the no puppy and 15-minute groups, but a negative relationship in the 30-minute group. This suggests that the researcher is correct that happiness scores depend on a combination of treatment and their love of puppies, we'll return to this plot later.

## `r user_visor()` Step 3: Fit the model  [(B)]{.lbl}

A simplified representation of our model is

$$
\text{happiness}_i = \hat{b}_0 + \hat{b}_1\text{dose}_i+ \hat{b}_2\text{puppy love}_i + \hat{b}_3[\text{dose} \times \text{ puppy love}]_i + e_i
$$

That is, the happiness score for an individual is predicted from the puppy therapy group to which they were assigned, their love of puppies, and the combined effect of these predictors.

However, the situation is more complicated because to include categorical predictors we must split the categories into dummy variables that represent comparison between the categories. One way to do this uses [dummy]{.kt} or [indicator]{.kt} coding in which each category is compared to a single reference category. Group membership is expressed in terms of 0s and 1s, and, in this example we might choose to compare each therapy condition to the no puppies group (Table 1). These are [non-orthogonal contrasts]{.alt}, which means they are not independent (because they both use the no puppies group scores).

```{r dummy_tbl, echo = FALSE, results = 'asis'}
dummy_tbl <- tibble(
  `Group` = c("No puppies (control)", "15 minutes", "30 minutes"),
  `Contrast 1 (long vs. none)` = c(0, 0, 1),
  `Contrast 2 (short vs. none)` = c(0, 1, 0)
  )
...
Alternatively, we can use contrast coding, where we define a set of [orthogonal contrasts]{.kt} which means that they are independent. For example, in `discovr_11` we used the contrast coding in Table 2.
...
In which the variables `contrast 1` and `contrast 2` are the dummy variables that represents the no puppies group compared to all other groups (`contrast 1`), and the difference between the 15-minute group and the 30-minute group (`contrast 2`). These contrasts work for the current example, because it extends the example from `discovr_11` so let's roll with these contrasts.

To add the interaction we multiply each contrast by the continuous predictor to get the following model

$$
\begin{aligned}
\text{happiness}_i = \ &\hat{b}_0 + \hat{b}_1\text{contrast 1}_i+ \hat{b}_2\text{contrast 2}_i \\
\quad &+ \hat{b}_3\text{puppy love}_i  + \hat{b}_4[\text{contrast 1} \times \text{puppy love}]_i \\
\quad &+ \hat{b}_5[\text{contrast 2} \times \text{puppy love}]_i + e_i
\end{aligned}
$$

Note our interaction term is split into two: each contrast variable multiplied by the continuous predictor (`puppy_love`).


#### `r robot()` Code example

We set these contrasts for `dose` using the `contrasts()` function (go back over `discovr_11` if this code doesn't make sense to you):
...
contrasts(pupluv_tib$dose) <- cbind(puppy_vs_none, long_vs_short )
...
Having set the contrasts we can build up the model starting with the intercept, then creating models that add each predictor and then the interaction.

#### `r alien()` Alien coding challenge

Use the code box to fit four models:

1. [intcpt_lm]{.alt}: `happiness` predicted from the intercept only
2. [pupluv_lm]{.alt}: `happiness` predicted from `puppy_luv`
3. [dose_lm]{.alt}: `happiness` predicted from `puppy_luv` and `dose`
4. [interact_lm]{.alt}: `happiness` predicted from `puppy_luv`, `dose` and their interaction.
...
```{r pupluv_mods, exercise = TRUE, exercise.lines = 5}

```

```{r pupluv_mods-hint-1}
# fit the first model (replace the xs):
lm(xxx ~ xxxx + xxxx, data = xxxx)
```

```{r pupluv_mods-hint-2}
intcpt_lm <- lm(happiness ~ 1, data = pupluv_tib)
# fit the second model (replace the xs):
pupluv_lm <- lm(happiness ~ xxxx, data = pupluv_tib)
```

```{r pupluv_mods-hint-3}
intcpt_lm <- lm(happiness ~ 1, data = pupluv_tib)=
pupluv_lm <- lm(happiness ~ puppy_love, data = pupluv_tib)
# fit the third model (replace the xs):
dose_lm <- lm(happiness ~ xxxx + xxxx, data = pupluv_tib)
```

```{r pupluv_mods-hint-4}
intcpt_lm <- lm(happiness ~ 1, data = pupluv_tib)=
pupluv_lm <- lm(happiness ~ puppy_love, data = pupluv_tib)
dose_lm <- lm(happiness ~ puppy_love + dose, data = pupluv_tib)
# fit the final model (replace the xs):
lm(happiness ~ puppy_love + dose + xxxx, data = pupluv_tib)
```


```{r pupluv_mods-solution}
intcpt_lm <- lm(happiness ~ 1, data = pupluv_tib)
pupluv_lm <- lm(happiness ~ puppy_love, data = pupluv_tib)
dose_lm <- lm(happiness ~ puppy_love + dose, data = pupluv_tib)
interact_lm <- lm(happiness ~ puppy_love + dose + dose:puppy_love, data = pupluv_tib)
```

Executing this code will appear to do nothing but the models are created and stored so now we can evaluate them.
...
Use `test_wald()` to compare the models you just fitted.

```{r pupluv_fit, exercise = TRUE, exercise.lines = 4, exercise.setup = "pupluv_lm_setup"}

```

```{r pupluv_fit-solution}
test_wald(intcpt_lm, pupluv_lm, dose_lm, interact_lm) |> 
  display()
```
...
pupluv_lrt <- test_wald(intcpt_lm, pupluv_lm, dose_lm, interact_lm)
```


Crucially, adding the interaction term improves the fit significantly meaning that the `dose:puppy_love` interaction is significant, `r report_lrt(pupluv_lrt, row = 4)`. We should retain the final model ([interact_lm]{.alt})

#### `r alien()` Alien coding challenge

We can evaluate the model assumptions in the usual way using `check_model()`. try this in the code box.

```{r interact_lm_check, exercise = TRUE, exercise.lines = 3, exercise.setup = "pupluv_lm_setup", fig.height = 10}

```

```{r interact_lm_check-solution}
check_model(interact_lm)
```
...
Although the bottom-left plot seems to indicate multicollinearity between `dose` and the interaction, this is to be expected because the interaction is literally the values of dose multiplied by the values of puppy love.
</div>
...
The diagnostic plots suggest we should adjust the model using robust methods because of heteroscedasticity. Figure 3 tells us the action to take is to uise heteroscedasticity consistent standard errors. Use what you know already about `model_parameters()` to inspect the parameter estimates of the model [interact_lm]{.alt} using HC4 robust standard errors.

```{r interact_rob, exercise = TRUE, exercise.lines = 3, exercise.setup = "pupluv_lm_setup"}

```

```{r interact_rob-hint-1}
# Place the model name into model_parameters() from the parameters package
model_parameters(xxxx, vcov = "xxxx") |> 
  display()
```
 
```{r interact_rob-solution}
model_parameters(interact_lm, vcov = "HC4")|> 
  display()
```


```{r, echo = F}
interact_lm_par <- model_parameters(interact_lm, vcov = "HC4")
```


The interaction supersedes everything else, so let's focus on those two parameter estimates. The first tells us that the relationship between puppy love and happiness was not significantly different in those that had therapy and those that did not, `r report_pe(interact_lm_par, row = 5)`. The second tells us that the relationship between puppy love and happiness was significantly different in those that 30 minutes of therapy compared to those that had 15, `r report_pe(interact_lm_par, row = 6)`. The fact the parameter estimate is negative means that the slope for the relationship between puppy love and happiness was smaller in the 30 minute group than the 15 minute group, which we can see in our earlier plot where there was (a frankly weird and unpredicted) negative relationship in the 30 minute group where happiness declined as puppy love increased.

We might also want to break the interaction down and look at the simple slopes (that is, the relationship between `puppy_love` and `happiness` within each treatment group). We can do this using what we learnt in `discovr_10` about the `estimate_slopes()` function from [modelbased]{.pkg}. It takes the form
...
Within the function we replace [my_model]{.alt} with the model we have created ([interact_lm]{.alt}), replace [continuous_variable]{.alt} with the continuous predictor (`puppy_love`) and [categorical_variable]{.alt} with the grouping variable (`dose`). By default it will return 95% confidence intervals so we can omit this argument unless we want to use a different level. 

#### `r robot()` Code example

We can view the simple slopes for the current model by executing:

```{r, eval = F}
estimate_slopes(interact_lm,
                trend = "puppy_love",
                by = "dose",
                ci = 0.95) |> 
  display()
...
Use the code box below to obtain the simple slopes of `puppy_love` for each `dose`.

```{r pup_slope, exercise = TRUE, exercise.lines = 6, exercise.setup = "pupluv_lm_setup"}
          
```


```{r pup_slope-solution}
estimate_slopes(interact_lm,
                trend = "puppy_love",
                by = "dose",
                ci = 0.95) |> 
...
```{r, echo = F}
pup_slopes <- estimate_slopes(interact_lm,
                trend = "puppy_love",
                by = "dose",
                ci = 0.95)
```

In the no puppy group, `r report_ss(pup_slopes, row = 1)`, and the 15-minute group, `r report_ss(pup_slopes, row = 2)`, there were significant and strong positive relationships between loving puppies and happiness. However, in the 30-minute group there was a non-significant negative relationship between loving puppies and happiness, `r report_ss(pup_slopes, row = 3)`. The significant interaction seems to reflect that the positive relationship between loving puppies and happiness is 'wiped out' by 30 minutes of puppy therapy. One interpretation of this effect might be that the 30-minutes of puppy therapy raises everyone's happiness, which attenuates the relationship between loving puppies and happiness.

## `r user_visor()` Analysis of covariance [(B)]{.lbl}

The second type of model we'll look at that has both categorical and continuous predictors assumes that predictors do not interact. It is known as a [parallel-slopes model]{.kt} because the lack of interaction implies that the relationship between the covariate (continuous and predictor) and outcome is the same across all levels of the categorical predictor. This being so allows us to think of interpreting the effect of the categorical variable [adjusted for]{.kt} (or at average levels of) the effect of the covariate. In this situation we often follow the so-called ANOVA route in Figure 5.

Imagine our puppy-therapy researcher, in a moment of clarity, realised that happiness scores after therapy would depend upon happiness scores *before* therapy. People who are really unhappy before treatment, even if they become more happy, will have lower scores after therapy than those who went into therapy feeling happier. Some data reflecting this design are in [puptreat_tib]{.alt}. The data are very similar to the previous example (and that of `discovr_11`), in that we have the variables `id` (the participants id code), `dose` (1 = No puppies, 2 = 15 minutes, 3 = 30 minutes), and `post_happy` (the person's happiness on a scale from 0-10 *after* therapy). However, we have an additional variable `pre_happy` that is each person's happiness on a scale from 0-10 *before* therapy.

The process for fitting a model that compares means while adjusting for a continuous variable (covariate) differs a little from out usual process, but only a little:

- Step 1: as always explore the data. However, in addition, we want to assess the independence of the covariate (not a statistical requirement but helps with interpretation), and check for [homogeneity of regression slopes]{.kt} to see whether the assumption of parallel slopes is reasonable.
...
These steps will be really similar to the previous example, so we'll blitz them in a single exercise.

#### `r alien()` Alien coding challenge

View the data in [puptreat_explore]{.alt}, check the order of the levels of the variable `dose`, and create a table of summary statistics (with CIs) for both pre- and post-therapy happiness.

```{r puptreat_explore, exercise = TRUE, exercise.lines = 10}

```

```{r puptreat_explore-solution}
# view the data
puptreat_tib
# check the levels of dose
...
- Test for independence of the covariate
...
Plot a violin plot of happiness scores in the three puppy therapy groups. Plot the raw data and display each group in a different colour.
...
ggplot2::ggplot(puptreat_tib, aes(x = dose, y = post_happy, colour = dose)) +
  geom_point(position = position_jitter(width = 0.1), alpha = 0.6) +
  geom_violin(alpha = 0.2) + 
  stat_summary(fun.data = "mean_cl_normal", geom = "pointrange", position = position_dodge(width = 0.9)) +
  coord_cartesian(ylim = c(0, 10)) +
...
ggplot2::ggplot(puptreat_tib, aes(x = dose, y = pre_happy, colour = dose)) +
  ...
  coord_cartesian(ylim = c(0, 10)) +
  scale_y_continuous(breaks = 0:10) +
  labs(x = "Puppy therapy group", y = "Pre-therapy happiness (0-10)", colour = "Puppy therapy group") +
  theme_minimal()
```
...
ggplot2::ggplot(puptreat_tib, aes(x = pre_happy, y = post_happy, colour = dose, fill = dose)) +
  geom_point() + 
  geom_smooth(method = "lm") +
...
The relationship between pre- and post-treatment happiness is positive (and similar in strength) in all three groups adding support to our test of homogeneity of regression slopes.

## `r user_visor()` Step 3: Fit the model  [(B)]{.lbl}

Our model is very similar to the previous example except that we exclude the interaction terms:

$$
\begin{aligned}
\text{happiness}_i = \ &\hat{b}_0 + \hat{b}_1\text{contrast 1}_i+ \hat{b}_2\text{contrast 2}_i \\
\quad &+ \hat{b}_3\text{pre-therapy happiness}_i + e_i
\end{aligned}
$$

That is, the happiness score for an individual is predicted from the puppy therapy group to which they were assigned,  and their pre-therapy happiness.

As mentioned earlier, typically when evaluating this sort of model we want to use *F*-statistics to evaluate all effects *simultaneously* and not *sequentially* like we do when we use `test_wald()`. To do this, we must use Type III sums of squares. Type III sums of squares differ from Type I in that all effects are evaluated taking into consideration all other effects in the model (not just the ones entered before). As such, the order that you specify predictors doesn't affect the results. (There's a more nuanced discussion of sums of squares in the book.)
...
For Type III sums of squares to be correctly calculated you must specify an orthogonal contrast for any categorical predictors. Orthogonal is just a posh word for independent. If you follow the rules for contrast coding in `discovr_11` and the book, you will get orthogonal contrasts. Also,  Helmert contrasts obtained using `contr.helmert()` are orthogonal (but might not compare the groups that interest you).
...
The contrasts we used in the previous example are orthogonal, and the hypotheses haven't changed so we can use the same contrast coding as the previous example to get two contrasts:
...
Use the code from the earlier example to set the contrasts in Table 2 for `dose`.

```{r set_contrast2, exercise = TRUE, exercise.lines = 5}
...
The conclusions are the same as the earlier example so check back to remind yourself.
...
Moving onto the dummy variables, the first one ([dose (puppy_vs_none)]{.opt}) compares the control group with the 15- and 30-minute groups combined. Using the contrast codes that we have used, the *b*-value is the difference between the adjusted mean of the control group and the average of the adjusted means for the 15- and 30-minute groups. The effect is significant, `r report_pe(puptreat_par, row = 3)`, indicating that the control group was significantly different from the combined adjusted mean of the puppy therapy groups. The *b*-value for the second dummy variable ([dose (long_vs_short)]{.opt}) is the difference between the adjusted means of the 15- and 30-minute groups. This effect is not significant, 
`r report_pe(puptreat_par, row = 4)`, indicating that the 30-minute group did not produce significantly different happiness than the 15-minute group after adjusting for love of puppies.
...
We can obtain *post hoc* tests in the same way that we did in `discovr_11` when we fitted the same model but without the covariate. Basically, we use the `estimate_contrasts()` function.
