...
## `r bmu()` Fit the model [(A)]{.lbl}

### `r bmu()` Dummy coding in `r rproj()` [(A)]{.lbl} 
...
This is, basically, what we do except that when we're using the linear model to compare means, we often want an overall test of each predictor. We don't get this with `broom::glance()` (we get only the overall fit). Instead we can use the `anova()` function to get overall effects of the model and then pipe this into `parameters::model_parameters()` to get an effect size ($\omega^2$) for each predictor.
...
## `r user_visor()` Contrast coding [(B)]{.lbl}
...
Were going to use the `estimate_contrasts()` function from the [modelbased]{.pkg} package [@R-modelbased], which builds upon the `emmeans` package [@R-emmeans] to provide more user-friendly experience. 
...
anova(xxxx) |> 
  parameters::model_parameters(xxxxxx)
```
