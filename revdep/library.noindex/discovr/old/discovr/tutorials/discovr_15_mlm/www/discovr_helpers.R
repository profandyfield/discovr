...
report_lrt <- function(lrt_obj, row = 2, digits = 2, p_digits = 3){
  lrt_obj <- tibble::as_tibble(lrt_obj)
  nrow <- nrow(lrt_obj)
  dp <- paste0("%.", digits, "f")
  p_dp <- paste0("%.", p_digits, "f")
  ...
  lrt_row <- lrt_obj |>
    dplyr::mutate(
      p = ifelse(p < 0.001, "*p* < 0.001", paste("*p* =", sprintf(p_dp, p))),
      dplyr::across(
        where(is.numeric), \(x) ifelse(grepl("\\.", x), sprintf(dp, x), x)
      ),
      row_no = 1:nrow
  ...
  paste0("$\\chi^2$(", lrt_row$df_diff, ") = ", lrt_row$Chi2, ", ", lrt_row$p)
}
