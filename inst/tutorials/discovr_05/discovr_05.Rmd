---
title: "discovr data visualization"
author: "Andy Field"
output:
  learnr::tutorial:
    progressive: false
...
* `sex`: the sex at birth  of the participant
...
This command creates an object called [exam_plot]{.alt} using the data in [exam_tib]{.alt}, and uses the `aes()` function to specify that **anxiety** is plotted on the *x*-axis and **exam_grade** on the *y*. We'd then need to add `geom_point()` to represent the data points:
...
Add `facet_wrap(~sex)` in the box below so that data for people male and female at birth are plotted in separate panels:
...
We can also specifying different colours for those male and female at birth using [colour = sex]{.alt} when we set up the plot:

```{r, eval = FALSE}
exam_plot <- ggplot(exam_tib, aes(anxiety, exam_grade, colour = sex))
```

To colour the interval around the line by sex, we'd also need to include [fill = sex]{.alt}:
