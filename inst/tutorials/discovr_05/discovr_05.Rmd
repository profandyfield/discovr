...
## `r bmu2()` Boxplots (aka Box-Whisker plots)
...
Note that all I have done is to add `fill = strategy` to the initial aesthetic. The rest of the command stays the same.

#### `r alien()` Alien coding challenge

Your original code is reproduced below, adapt it to include `fill = strategy` and run it. Compare the plot to the previous version. Note that the plot still splits the data by **time** along the *x*-axis, but within each category the data from the wishing on a star group is shown in a different colour to the data from the hard work group.
...
Like with `fill`, we adapt the first line of code, but this time to include `colour = strategy`:
...
Add `colour = strategy` to the code below and see what happens when you run it.
...
### `r bmu2()` Grouping using `facet_wrap()`
...
In the `stat_summary()` function, we're asking `r rproj()` to calculate the means (`fun = "mean"`). The argument `geom = "point"` asks `ggplot2` to display the means as dots using `geom_point()`. The final argument, `size = 4`, determines the size of the dots and overrides the default (you can omit this argument if you like).
...
### `r bmu2()` Adjusting the position of geoms
...
### r user_visor()` Exploring layers

This is a good opportunity to remind you that each line of the command adds a layer to the plot *in the order you specify them*.


#### `r alien()` Alien coding challenge

Add the line `violin_plot() +` directly **below** the line that specifies `stat_summary()`.
...
Now add the line `violin_plot() +` directly **above** the line that specifies `stat_summary()`
...
### `r user_visor()` Changing the appearance of points
...
We could also change the shape of the geom. Figure 2 shows the numbers representing particular shapes.
...
will make the points blue (`colour = "#56B4E9"`), larger than default (`size = 4`), triangles (`shape = 3`) and slightly transparent (alpha = 0.8). 
