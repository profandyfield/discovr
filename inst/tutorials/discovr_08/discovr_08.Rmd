...
<div class="tip">
  ...
  If we want all 6 plots we specify this using `which = 1:6`.
</div>

### `r bmu2()` Residual plots

We use residuals plots (plots 1 and 3) to look for linearity and homoscedasticity. The Figure below (taken from [@field_discovering_2021]) shows that we're looking for a random scatter of dots. Curvature in the plot indicates a lack of linearity, and a funnel shape (residuals that 'fan out') indicates heteroscedasticity. 

```{r, out.width = "600px", fig.cap = "Figure 2: Residual plots", echo = FALSE}
knitr::include_graphics("./images/dsr2_fig_06_29_pred_resid.png")
```

Let's look at the plots for our model to see whether we can see any of these patterns.

#### `r bug()` Exercise

Using the code example above, obtain plots 1 and 3 for the model `album_full_lm`:
...
Using the code example above, obtain plot 2 for the model `album_full_lm`:
...
### `r user_astronaut()` Pretty residual plots
...
For the normality plot we can use the same code but change `which = c(1, 3)` to `which = 2`. Try this below.
...
### `r bmu2()` Influential cases and outliers: plots 
...
### `r user_visor()` Influential cases and outliers: numbers 
...
## `r user_visor()` Robust linear models
...
### `r user_astronaut()` Robust parameter estimates
...
### `r user_astronaut()` Robust confidence intervals and significance tests
