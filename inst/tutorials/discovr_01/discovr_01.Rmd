...
Note that we load the `dplyr` package to make all of the functions from that package available to `r rproj()` and subsequently omit the `dplyr::` before the `select()` function. Most code that you'll see is concise, but as a new `r rproj()` user I think there are benefits to using an explicit style.
