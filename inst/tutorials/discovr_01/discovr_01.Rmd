...
```
This command creates an object called `metalli_tib` (I tend to use `_tib` to denote a tibble) that contains all of the variables we have created in columns. You view tibbles by executing its name. The contents of the tibble will be printed within the R markdown document itself below the code chunk. By default the first 10 rows will be displayed and as many columns as the width of the pane allows. The display is interactive so you can navigate across columns or down beyond the first 10 rows (withinin this tutorial it won't be interactive!).

Create a tibble of the metallica data:
...
`instrument == "Drums" | instrument == "Bass guitar"`. The `|` denotes 'or', so this command would read as ‘the value of the variable instrument is equal to the word ‘Drums’ OR the value of the variable instrument is equal to the phrase ‘Bass guitar’. Again, we’d insert this statement into the `filter()` function.
```
metalli_tib %>% 
