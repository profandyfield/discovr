...
The same but using a tidyverse approach:
...
If we predicted a negative correlation, we could add in the alternative argument:

```{r}
cor.test(exam_tib$exam_grade, exam_tib$anxiety, alternative = "less")
```

We could also specify a different confidence interval than 95%:

```{r}
cor.test(exam_tib$exam_grade, exam_tib$anxiety, conf.level = 0.99)
```

The relationship between revision and exam grade:

```{r}
cor.test(exam_tib$revise, exam_tib$exam_grade)
```

The relationship between revision and exam anxiety:
```{r}
...
A quick aside, let's look at the code for writinga function to print the mean:
