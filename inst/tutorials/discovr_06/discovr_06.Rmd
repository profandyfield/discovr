...
In situations like this, where we want to convert multiple variables, it can be quicker to use `dplyr::across()` within `mutate()` to apply our function `make_z()` to multiple variables simultaneously. The function `across()` applies a mutation across certain columns (hence, the name).
...
download_tib <- download_tib %>% 
  dplyr::mutate(
    dplyr::across(day_1:day_3, list(make_z), .names = "{.col}_z")
  )
...
This code pipes the data ([download_tib]{.alt}) into the `mutate()` function, inside which it goes into the `across()` function, where two things happen. First, we select the variables to mutate across. We specify the variables as [day_1:day_3]{.alt}, which means all the variables in the tibble starting with **day_1** and finishing with **day_3**. Second, we tell `across()` what function we want to apply to these variables. In this case, we want to apply the function that we wrote called `make_z()`. Basically, variables **day_1**, **day_2** and **day_3** get passed through the `make_z()` function simultaneously. By including [.names = "\{.col\}_z"]{.alt} within `across()`, the resulting variables will be named after the original columns ([\{.col\}]{.alt} refers to the original column names) but with a ‘z’ appended (so, ["\{.col\}_z"]{.alt} means *take the original column name and add "_z"*). The new variables (**day_1_z**, **day_2_z** and **day_3_z**) will be stored as the final three columns in the tibble.
