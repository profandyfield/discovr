...
### `r bmu()` Interpreting the model [(A)]{.lbl}
...
### `r user_visor()` Effect size for independent means [(B)]{.lbl}

We can use the [effectsize]{.pkg} package [@effectsize2020; @R-effectsize] to calculate Cohen's *d*. There are three useful functions here:
...
We can put the output of [effectsize]{.pkg} through `kable()` to round values in our report. For example, to round to a maximum of 3 decimal places we could use the code below. Try it in the previous code box.
...
We conduct a paired *t*-test in a similar way to an independent *t*-test except that we (1) we cannot use the formula `outcome ~ predictor` and instead have to specify the data for the two conditions separately; and (2) we use [paired = TRUE]{.alt} in the `t.test()` function. The general form of the function is, therefore,
 
```{r, echo = T, eval = F}
new_model <- t.test(data_for_condition_1,
...
* [data_for_condition_1]{.alt}: is replaced with code that obtains the scores for the first condition.
* [data_for_condition_2]{.alt}: is replaced with code that obtains the scores for the second condition.
* [paired = FALSE]{.alt}: by default scores are treated as independent ([paired = FALSE]{.alt}), but for a repeated measures design we want to treat scores as dependent so we need to change this to [paired = TRUE]{.alt}.

The other arguments are the same as for the independent *t*-test.

To do a paired *t*-test requires the data to be in messy format, which is annoying given how much I've gone on about using tidy data. Worry not, because in [discovr_06]{.alt} we used the `pivot_wider()` function to turn tidy data into messy data. Naturally you  won't remember that far back, but we can create a messy version of the repeated measures data using the following code:
...
Now we have the data in messy format, adapt the code for the `t.test()` function to run a dependent *t*-test and save the object as [cloak_rm_mod]{.alt}
...
cloak_rm_mod <- t.test(cloak_messy_tib$Cloak, cloak_messy_tib$`No cloak`, paired = TRUE)
cloak_rm_mod
```

As before, this code creates a model called [cloak_rm_mod]{.alt} based on predicting mischief scores (**mischief**) from group membership (**cloak**). We can view this model by executing its name (hence the second command).
...
To compare dependent means with a robust model we can use the `yuend()` function from the `WRS2` package [@Mair_Wilcox_2019] to get a robust test based on (20% by default) trimmed means. It takes the general form:

```{r, eval = F}
WRS2::yuend(scores_condition_1, scores_condition_2,  tr = .2)
```

In other words, like for the dependent *t*-test, we need to specify the scores for each condition as separate variables. Luckily we already have the data in messy format in `cloak_messy_tib`.

#### `r robot()` Code example

Using the data in `cloak_messy_tib` and the `yuend()` function (assuming we don't want to change from a 20% trim) we'd can run a robust test on dependent means using:
...
## `r user_astronaut()` Bayesian approaches [(C)]{.lbl}
...
### `r user_astronaut()` Evaluating independent means [(C)]{.lbl}
...
### `r user_astronaut()` Evaluating dependent means [(C)]{.lbl}
