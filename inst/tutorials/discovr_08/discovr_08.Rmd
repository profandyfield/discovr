...
Hi, welcome to **discovr** space pirate academy. Well done on embarking on this brave mission to planet `r rproj("tip")`s, which is a bit like Mars, but a less red and more hostile environment. That's right, more hostile than a planet without water. Fear not though, the fact you are here means that you *can* master `r rproj("tip")`, and before you know it you'll be as brilliant as our pirate leader Mae Jemstone (she's the badass with the gun). I am the space cat-det, and I will pop up to offer you tips along your journey.
...
* Before attempting this tutorial it's a good idea to work through [this tutorial on how to install, set up and work within R and RStudio](http://milton-the-cat.rocks/learnr/r/r_getting_started/).

* The tutorials are self-contained (you practice code in code boxes). However, so you get practice at working in RStudio I strongly recommend that you create an `r rproj()` markdown file within an Rstudio project and practice everything you do in the tutorial in the `r rproj()` markdown file, make notes on things that confused you or that you want to remember, and save it. Within this markdown file you will need to load the relevant packages and data. 
...
We can load all of the packages that are exceptions in one step by loading `tidyverse` at the beginning of our R Markdown document:
...
Set up an RStudio project in the way that [I recommend in this tutorial](http://milton-the-cat.rocks/learnr/r/r_getting_started/#section-working-in-rstudio), and save the data files to the folder within your project called `data`. Place this code in the first code chunk in your `r rproj()` Markdown document:
...
## `r bmu2()` The linear model process
...
## `r bmu2()` One predictor
### `r bmu2()` Fitting the model
...
Based on the code above, we can fit the model with the code below. Because there are no missing values in the data the `na.action = na.exclude` is optional. Try executing this code.

```{r fit_album, exercise = TRUE, exercise.lines = 2}
album_lm <- lm(sales ~ adverts, data = album_tib, na.action = na.exclude)
```

You might think that you've made a mistake because nothing much happens. In fact, we have created an object called `album_lm` that has the model stored, and we can extract information from it. The reason nothing has happened is because we haven't asked `r rproj()` to show us anything. Let's look at how to do that.

<div class="tip">
  `r cat_space()` **Tip:**
  
  I tend to name linear models with the suffix _lm but you don’t have to share my obsession with using suffixes that tell me what the object contains.
  
</div>


###	`r bmu2()` Extracting model information with `summary()` 
...
This code example shows how to get the summary information fpor the model that we just fitted.
...
###	`r bmu2()` Overall fit of the model`

To get overall model fit statistics we place out model (`album_lm`) into `glance()`.
...
<div class="tip">
  `r cat_space()` **Tip: scientific notation**
   ...
   Scioentific notation can seem confusing, but e-x is shorthand for $\times 10^{-x}$ and e+x is shorthand for $\times 10^{x}$. Here's two specific examples:
...
### `r user_visor()` Exploring the standard error of *b* (optional)
...
### `r user_visor()` Confidence intervals for *b*
...
See if you can adapt the previous code (reproduced in the code box) to save the model as `album_full_lm` and to include the additional predictors
...
This code creates a model in which **sales** is predicted from **advertising**, **airplay** and the band’s **image** and stores it in an object called `album_full_lm`. Again nothing seems to have happened, but actually the model has been created and stored. Next we need to extract information from it.
...
Adapt the previous code for `glance()` to view the full model (i.e. `album_full_lm`)
...
Use `tidy()` instead of `summary()` to extract the model parameters for `album_full_lm`. Don't forget the confidence intervals.
...
### `r user_visor()` Confidence intervals
...
## `r bmu2()` Unguided Examples
### `r bmu2()` Metal and mental health

<div class="tip">
  `r cat_space()` **Advisory**
...
### `r user_visor()` Predicting social anxiety
...
soc_anx_lm <- lm(spai ~ tosca, data = my_tibble, na.action = na.exclude)
...
# Now compare the models using anova() in combination with tidy()
```
...
In this section we look at whether the model we have just fitted is biased.
...
In other words, you place the name of your model into the function and use which if you want to override the default plots (more on that in a minute). For our final model, we could simply execute:

```{r eval = FALSE}
plot(album_full_lm)
```

The `plot()` function can produce the six plots described below:

1. The predicted values from the model (*x*-axis) against the residuals (*y*-axis). You want to see a random pattern of dots with no funnelling and no non-linear patterns. The red line should be roughly flat. If you see funnelling and the red line shows an upwards or downward trend then you have heteroscedasticity.
2. A Q-Q plot of the standardized residuals. You want the dots to fall along the diagonal line.
3. The predicted values from the model (*x*-axis) against the square root of the standardized residuals (*y*-axis). This plot is interpreted in the same way as plot 1.
...
5. The leverage value for each case (*x*-axis) against the standardized residual (*y*-axis). You want to see a flat red line indicating that leverage does not vary as a function of the standardized residual (i.e. there are no cases with a large residual and large leverage). This plot is useful for identifying cases with large values of Cook's distance too because it superimposes red dotted lines representing Cook's distances of 0.5 and 1. Any cases falling outside of the red dotted boundary for a Cook's distance of 1 are likely to be influential cases.
6. The leverage value for each case (*x*-axis) against the corresponding Cook’s distance (*y*-axis). There are also contours representing standardized residual boundaries of 2, 2.5, 3, 3.5 (the approximate criteria we have used in this chapter). We’re looking for cases with a large residual and large leverage and/or Cook’s distance.

By default, `plot()` displays plots 1, 2, 3 and 5, but we can force it to produce all 6 plots by using the which argument to specify the plots we want. If we want all of the plots, 6 we can specify this as `which = 1:6`.

```{r eval = FALSE}
plot(album_full_lm, which = 1:6)
```

Similarly, if we want to produce one of the plots in isolation, we can use the which argument in conjunction with the number of the plot we want. For example, to get plot 3 we would execute:
...
The Figure below (taken from [@field_discovering_2021]) shows that we're looking for a random scatter of dots. Curvature in the plot indicates a lack of linearity, and a funnel shape (residuals that 'fan out') indicates heteroscedasticity. 

```{r, out.width = "600px", fig.cap = "Figure 2: Residual plots"}
knitr::include_graphics("./images/dsr2_fig_06_29_pred_resid.png")
```

Let's look at the plots in our output to see whether we can see any of these patterns. Using what we learnt above, we can obtain these plots by asking for plot 1 and 3:

```{r eval = FALSE}
plot(album_full_lm, which = c(1, 3))
```

Try this code in the box below.

```{r zpred_resid, exercise = TRUE, exercise.lines = 2}
...
```{r album_qq, exercise = TRUE, exercise.lines = 2}
...
```{r quiz_normal_ex1}
quiz(
  question("Based Q-Q plot, can we assume normality of the residuals?",
    answer("Yes", correct = TRUE, message = "The distribution is very normal: the histogram is symmetrical and approximately bell-shaped and the dots in the P-P plot lie almost exactly along the diagonal, which   indicates a normal distribution."),
    answer("No", message = "The distribution is very normal: the histogram is symmetrical and approximately bell-shaped and the dots in the P-P plot lie almost exactly along the diagonal, which   indicates a normal distribution."),
    correct = "Correct - well done!",
    random_answer_order = TRUE,
    allow_retry = T
  )
...
### Pretty residual plots (optional)

With the `ggfortify` package loaded, you could execute

```{r eval = FALSE}
ggplot2::autoplot(album_full_lm, which = 1)
```

to produce a nicely formatted plot of the predicted values against the residuals. More important, the resulting plot is a ggplot object, which means that you can do things to it using ggplot2 functions. For example, you can apply a theme:
...
Also, some useful arguments are added to `autoplot()`, for example, you can use colour to set a colour in the usual way for the points plotted (the code below uses [hex code](https://www.color-hex.com/) "#5c97bf" to set the `colour` to blue) and `smooth.colour` to set the colour of the trend line on the plots (set as hex code "#ef4836"):
...
You can also apply familiar `ggplot2` aesthetics such as `size` (to adjust the size of the dots plotted) and `alpha` (to set transparency of the data points):
...
For example, we could get prettier versions of our residual plots using
...
Try this in the code box (and try adjusting the hex colours and sizes)

```{r zpred_resid_pretty, exercise = TRUE, exercise.lines = 8}

```

```{r zpred_resid_pretty-solution}
ggplot2::autoplot(album_full_lm,
                  which = c(1, 3),
                  colour = "#5c97bf",
                  smooth.colour = "#ef4836",
...
For the normality plot we can use the same code but simply adjust `which = c(1, 3)` to `which = 2`:
...
Try this in the code box (and again try adjusting the hex colours and sizes)

```{r albumqq_pretty, exercise = TRUE, exercise.lines = 8}
...
As a bare minimum we should check the model for outliers using the standardized residuals and use Cook's distance to identify influential cases. [@field_discovering_2021] describes a much wider battery of values that you can use to check for these things so if you're starting to get the stats bug (?!) then check that out. Otherwise, we can apply the following heuristics:
...
A quick way to check for influential cases is to inspect the influence plots for the model, which we can obtain by selecting plots 4 to 6 from the `plot()` function (described earlier):

```{r eval = FALSE}
plot(album_full_lm, which = 4:6)
```

Again you can get prettier versions using: 
...
Try one (or both) of these code in the box below.

```{r album_inf_plot, exercise = TRUE, exercise.lines = 8}

```

```{r album_inf_plot-solution}
plot(album_full_lm, which = 4:6)
...
```{r quiz_casewise_ex1}
quiz(
  question("What percentage of cases have standardized residuals with absolute values greater than 2?",
    answer("6%", correct = TRUE, message = sprintf("12 cases have standardized residuals with absolute values greater than 2, and $\\frac{12}{200} \\times 100 = 6$")),
    ...
    answer("0.06", message = "This is the proportion of cases that have standardized residuals with absolute values greater than 2, not the percentage. Multiply the value by 100 to get the correct answer."),
    correct = "Correct - well done!",
    random_answer_order = TRUE,
    allow_retry = T
  ),
  question("What percentage of cases have standardized residuals with absolute values greater than 2.5?",
    answer("1%", correct = TRUE, message = sprintf("2 cases has standardized residuals with absolute values greater than 2, and $\\frac{2}{200} \\times 100 = 1$")),
    answer("3%", message = "You have only considered the positive standardized residuals. Remember that the absolute values means the value when you ignore the plus or minus sign."),
    answer("0.01", message = "This is the proportion of cases that have standardized residuals with absolute values greater than 2, not the percentage. Multiply the value by 100 to get the correct answer."),
    answer("0.5", message = "Did you forget to include the case with the residual of 3.061 or the one with a resodual of -2.577?"),
    correct = "Correct - well done!",
    random_answer_order = TRUE,
    allow_retry = T
  ),
...
![Figure 10: Values of Cook's distance in the data editor](./images/lm03_fig_10_cooks.png)


```{r quiz_cooks_ex1}
quiz(
  question("Are there any Cook's distances greater than 1 in the data editor?",
    ...
    answer("Yes", message = "Are you sure? There really shouldn't be any!"),
    correct = "Correct - well done!",
    random_answer_order = TRUE,
    allow_retry = T
  )
...
The next output contains the bootstrap confidence intervals for each model parameter. These bootstrap confidence intervals^[Because of how bootstrapping works the values in your output will be different than mine, and different if you re-run the analysis.] and significance values do not rely on assumptions of normality or homoscedasticity, so they give us an accurate estimate of the population value of *b* for each predictor (assuming our sample is one of the 95% with confidence intervals that contain the population value).


![Output 2: Bootstrapped model parameter estimates](./images/lm03_out_02.png)
...
quiz(
  ...
  question("The bootstrap confidence interval for image ranges from 6.26 to 15.28. What does this tell us?",
    ...
    answer("There is a 95% chance that the population value of *b* lies between 6.26 and 15.28.", message = "You cannot make probability statements from a confidence interval. We don't know whether this particular CI is one of the 95% that contains the population value of *b*."),
    answer("The probability of this confidence interval containing the population value is 0.95.", message = "The probability of this confidence interval containing the population value is either 0 (it doesn't) or 1 (it does) but it's impossible to know which."),
    answer("I can be 95% confident that the population value of *b* lies between 6.26 and 15.28.", message = "Confidence intervals do not quantify your subjective confidence."),
    correct = "Correct - well done!",
    random_answer_order = TRUE,
    allow_retry = T
  ),
...
* The tutorials typically follow examples described in detail in [@field_discovering_2021], so that book is an obvious place to go for further reading.
