...
In situations like this, where we want to convert multiple variables, it can be quicker to use `dplyr::across()` within `mutate()` to apply our function `make_z()` to multiple variables simultaneously. Put simply the function applies a mutation across certain columns (hence, the name).
