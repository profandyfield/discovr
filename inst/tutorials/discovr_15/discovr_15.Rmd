...
description: "Repeated measures designs. Fitting models for one- and two-way repeated measures designs using the afex package. This tutorial builds on previous ones to show how models can be fit with one or two categorical predictors when these variables have been manipulated within the same entities. We look at fitting the models, setting contrasts for the categorical predictors, obtaining estimated marginal means, interaction plots, simple effects analysis, diagnostic plots, robust models and Bayes factors."
...
However, we might also want to capture individual differences in the effects of **entity** and **scent**, by adding in terms that represent the difference in vocalizations for a particular dog from the overall effect of **entity** ($\hat{u}_{Ej}$) and **scent** ($\hat{u}_{Sj}$):

$$
\begin{aligned}
\text{vocalizations}_{ij} & = \hat{b}_{0j} + \hat{b}_{1}\text{entity}_{ij} + \hat{b}_{2}\text{scent}_{ij} + \hat{b}_{3}(\text{entity}_{ij}\times\text{scent}_{ij}) + e_{ij}\\
\hat{b}_{0j} & = \hat{b}_0 + \hat{u}_{0j} + \hat{u}_{Ej} +  \hat{u}_{Sj}\\
\end{aligned}
$$

These terms will each have an associated parameter that estimates the variance (across dogs) of the effect of **entity** ($\hat{\sigma}^2_{\mu_E}$) and **scent** ($\hat{\sigma}^2_{\mu_S}$).

This is a suitable model when we want to treat our factors as fixed effects. However, there will be situations where you want to treat a factor as a random effect. For example, our scents were strategically chosen (none, human scent, fox scent), but what if we hadn't started with specific ideas of what scents might effectively mask the entity's smell. Instead, we chose (arbitrarily) a selection of perfumes, and we might want to generalize findings beyond these perfumes (because there was no particular reason we selected them). In this case, we might want to fit a random effect for **scent** and we could do this in the usual way by changing the parameter associated with it ($\hat{b}_{2}$) so that it varies across dogs ($\hat{b}_{2j}$) and include the associated term to reflect these individual differences in dogs ($\hat{u}_{2j}$). At the extreme, we could fit a model with all random effects:

$$
\begin{aligned}
\text{vocalizations}_{ij} & = \hat{b}_{0j} + \hat{b}_{1j}\text{entity}_{ij} + \hat{b}_{2j}\text{scent}_{ij} + \hat{b}_{3j}(\text{entity}_{ij}\times\text{scent}_{ij}) + e_{ij}\\
\hat{b}_{0j} & = \hat{b}_0 + \hat{u}_{0j} \\
...
This model introduces a lot of complexity into the model and possibly too many parameters for the data to support. So, we will stick to an analogue of analysis of variance and only include a random intercept overall and for each of the two factors (i.e. the first equation above).
