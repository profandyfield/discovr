...
We can obtain models based on robust standard errors by setting [vcov = TRUE]{.alt} and using the argument [vcov.type]{.alt} to set which method to use. If we omit the argument it defaults to ["HC3"]{.alt} (which is fine), but we can change it to ["HC4"]{.alt} (which some consider better). This code makes our model robust by implementing HC4 standard errors.

```{r hc4, exercise = TRUE, exercise.lines = 3, exercise.setup = "album_lm_setup"}
parameters::model_parameters(album_full_lm, vcov = TRUE, vcov.type = "HC4") |> 
  knitr::kable(digits = 3)
```
...
parameters::model_parameters(album_full_lm, vcov = TRUE, vcov.type = "HC3") |> 
  knitr::kable(digits = 3)
```

```{r hc3-hint-2}
# or omit the vcov.type argument altogether (because it defaults to HC3)
parameters::model_parameters(album_full_lm, vcov = TRUE) |> 
  knitr::kable(digits = 3)
```
