...
newz_imps <- newz_md_tib %>% 
  mice::mice(method = "pmm", m = 5)

ids <- c("R_tEANRrR9dhnELbH", "R_UYHM5dj6jNuwnyV", "R_1Nac0KH2yUIf8FL", "R_8uMt5tRNiusnHTr", "R_3rM7LolOYmX4RkG")

newz_md_tib_sample  <- newz_md_tib %>% 
...
newz_md_tib_imps <- newz_imps %>% mice::complete(., "long") %>% 
  dplyr::filter(id %in% ids) %>%  
  dplyr::mutate_if(
    vars(is.numeric(.)),
    list(~round(., 2))
...
newz_md_tib_sample %>% 
  dplyr::bind_rows(., newz_md_tib_imps) %>% 
  readr::write_csv(., here::here("/data-raw/dev_figures/multiple_imputation_data.csv"))
...
newz_fit <- with(data = newz_imps, exp = lm(fake_newz ~ 0 + delusionz + thinkz_open + thinkz_anal))
newz_fit$analyses

summary(mice::pool(newz_fit))
```
