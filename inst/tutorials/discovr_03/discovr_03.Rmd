...
description: "Confidence intervals: interactive app demonstrating what a confidence interval is, computing nromal and bootstrap confidence intervals using R, adding confidence intervals to data summaries."
...
## `r bmu()` Confidence intervals in R [(1)]{.alt}

### `r bmu()` A basic confidence interval [(1)]{.alt}

Let’s compute a confidence interval using `r rproj()`. In the book we have the number of followers for 11 Facebook users:
...
Use what you have learnt in previous tutorials to enter these scores into a variable called **friends** stored within a tibble called [fb_tib]{.alt}.

```{r fb_dat, exercise = TRUE, exercise.lines = 4}

```

```{r fb_dat-hint-1}
# There are numerous ways to do this task, this is the most efficient (probably)
# You want to name the object fb_tib so start with ...
fb_tib <- ...
```
...
fb_tib <- tibble::tibble(???????)
# Within this function create the variable friends
```

```{r fb_dat-hint-3}
# Create the variable friends
fb_tib <- tibble::tibble(
  friends = ??????
)
```
...
In this example, we want the 95% confidence interval for the variable **friends** within the tibble [fb_tib]{.alt}, so we could use the example code and replace [object]{.alt} with [fb_tib$friends]{.alt}. Try this out:

```{r fb_ci, exercise = TRUE, exercise.lines = 2}
...
Adapt the code from the previous challenge to get the 90% CI of the variable **friends**.
  
```{r fb_ci_90, exercise = TRUE, exercise.lines = 2}

```

```{r fb_ci_90-solution}
ggplot2::mean_cl_normal(fb_tib$friends, conf.int = 0.9)
``` 
 

### `r bmu()` Adding confidence intervals to summary tables [(1)]{.alt}

In [discovr_02]{.alt} we explored code to compile a table of summary statistics for a variable. To recap, we produced a summary table for these Facebook scores by executing:
...
You might wonder what the point of this is when the function itself spits out a summary table, but it is because it allows you to include other information in the table. For example, we could include all o fthe other summary statistics that we previously computed.

#### `r alien()` Alien coding challenge

Adapt the code below (which produces various summary statistics for the number of friends people have on social media) to include the mean, and lower and upper boundary of the 95% confidence interval.

```{r fb_summary, exercise = TRUE, exercise.lines = 14}
fb_tib %>%
  dplyr::summarize(
    median =  median(friends),
    range = max(friends) - min(friends),
...
question("The 95% confidence interval for friends ranges from 56.85 to 133.15. What does this tell us?",
    answer("If this confidence interval is one of the 95% that contains the population value then the mean number of friends in the population lies between 56.85 and 133.15.", correct = TRUE),
    answer("There is a 95% chance that the mean number of friends in the population lies between 56.85 and 133.15.", message = "You cannot make probability statements from a confidence interval. We don't know whether this particular CI is one of the 95% that contains the population value of the mean number of friends."),
    ...
    answer("I can be 95% confident that the mean number of friends in the population lies between 56.85 and 133.15.", message = "Confidence intervals do not quantify your subjective confidence."),
    correct = "Correct - well done!",
    random_answer_order = TRUE,
    allow_retry = T
)
...
For example, to get the robust confidence interval for the mean number of friends we could execute:


```{r fb_ci_boot, exercise = TRUE, exercise.lines = 2}
ggplot2::mean_cl_boot(fb_tib$friends)
```
...
Adapt the code below (which produces various summary statistics for the number of friends people have on social media) to include the mean, and lower and upper boundary of the 95% [bootstrap]{.alt} confidence interval.

```{r fb_summary_boot, exercise = TRUE, exercise.lines = 14}
fb_tib %>%
  dplyr::summarize(
    median =  median(friends),
    range = max(friends) - min(friends),
...
```{r fb_summary_boot-solution}
fb_tib %>%
  dplyr::summarize(
    Mean =  ggplot2::mean_cl_boot(friends)$y,
    `95% CI Lower` = ggplot2::mean_cl_boot(friends)$ymin,
    `95% CI Upper` = ggplot2::mean_cl_boot(friends)$ymax,
    median =  median(friends),
    range = max(friends) - min(friends),
    `lower quartile` = quantile(friends, probs = 0.25),
    `upper quartile` = quantile(friends, probs = 0.75),
