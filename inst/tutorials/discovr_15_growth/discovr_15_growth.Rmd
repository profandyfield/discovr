...
rehab_growth_tib <- here::here("data/zombie_growth.csv") %>%
  readr::read_csv() %>%
   dplyr::mutate(
    ...
    time = forcats::as_factor(time) %>% forcats::fct_relevel(., "Baseline", "1 month", "6 months", "12 months")
    )
```
...
In [discovr_01]{.alt} we learnt the following good advice
...
With the pre-loaded data I have been kind to you and set up categorical predictors in the datasets as factors with the levels coded conveniently for the hypotheses being tested. However, we should practice following the advice because life is sometimes cruel and other people's data life is usually cruel.

The data in [growth_evil_tib]{.alt} are the same as [rehab_growth_tib]{.alt} but have been given to you by a researcher who cares not for factor levels. Notwithstanding the frailty of human memory, we might recall that we can check the levels of the variables **intervention** and **time** by placing each one in the `levels()` function and executing.
...
$$
\begin{aligned}
\widehat{\text{resemblance}}_{ij} =& \big[\hat{\gamma}_{00} + \hat{\gamma}_{10}\text{time}_{ij} + \hat{\gamma}_{01}\text{intervention}_i + \hat{\gamma}_{11}(\text{intervention}_i \times \text{time}_{ij})\big] + \\
...
* Include [scale_x_continuous(breaks = c(0, 1, 6, 12), labels = c("0", "1", "6", "12"))]{.alt} to set breaks at each of the 4 time points and to assign them labels
...
To fit the model we use the `lme()` function from the `nlme` package [@pinheiro_nlme:_2021], because we want to model the dependency between scores within zombies (see the earlier description of the hierarchical data structure and Figure 1).

The `lme()` function was explained in [discovr_14]{.alt}. To recap, it takes the following general form (I've retained only the key options):
...
<div class="tip">
  `r cat_space()` **Tip: Formatting output**

If you have output that is a tibble, you can do things like round the values to 2 decimal places when you knit by piping it into `knitr::kable()` and setting [digits = 2]{.alt}. For example, if we want to round the values in the table generated by `anova()` we can knit the table with rounded values using
...
<div class="tip">
  `r cat_space()` **Tip: Degrees of freedom**
...
rehab_mod_fixed_intervention_f <- round(rehab_mod_fixed$`F-value`[3], 2)
rehab_mod_fixed_intervention_p <-round(rehab_mod_fixed$`p-value`[3], 2)
rehab_mod_fixed_time_f <- round(rehab_mod_fixed$`F-value`[2], 2)
rehab_mod_fixed_time_p <- round(rehab_mod_fixed$`p-value`[2], 2)
rehab_mod_fixed_int_f <- round(rehab_mod_fixed$`F-value`[4], 2)
```

The results show no significant main effect of **intervention**, *F*(`r rehab_mod_fixed$numDF[3]`, `r rehab_mod_fixed$denDF[3]`) = `r rehab_mod_fixed_intervention_f`, *p* = `r rehab_mod_fixed_intervention_p`. However, the main effect of **time_num** was significant, *F*(`r rehab_mod_fixed$numDF[2]`, `r rehab_mod_fixed$denDF[2]`) = `r rehab_mod_fixed_time_f`, *p* = `r rehab_mod_fixed_time_p`, and the **time_num** ×  **intervention** interaction, *F*(`r rehab_mod_fixed$numDF[4]`, `r rehab_mod_fixed$denDF[4]`) = `r rehab_mod_fixed_int_f`, *p* < 0.001.
...
```{r, echo = F}
rehab_mod_fe <- broom.mixed::tidy(rehab_mod, conf.int = T, effects = "fixed")
rehab_mod_re <- broom.mixed::tidy(rehab_mod, conf.int = T, effects = "ran_pars")
```


The final model shows that

* Overall gene therapy (compared to a wait list control) did not significantly predict resemblance scores, $\hat{\gamma}$ = `r round(rehab_mod_fe$estimate[3], 2)` [`r round(rehab_mod_fe$conf.low[3], 2)`, `r round(rehab_mod_fe$conf.high[3], 2)`], *t*(`r rehab_mod_fe$df[3]`) = `r round(rehab_mod_fe$statistic[3], 2)`, *p* = `r round(rehab_mod_fe$p.value[3], 2)`. Resemblance scores were, overall, `r round(rehab_mod_fe$estimate[3], 2)` percent (remember resemblance is measured on a percentage scale) lower after gene therapy than in the wait list. However, this effect ignores the change over time.
* Resemblance scores significantly changed over time $\hat{\gamma}$ = `r round(rehab_mod_fe$estimate[2], 2)` [`r round(rehab_mod_fe$conf.low[2], 2)`, `r round(rehab_mod_fe$conf.high[2], 2)`], *t*(`r rehab_mod_fe$df[2]`) = `r round(rehab_mod_fe$statistic[2], 2)`, *p* < .001. For every month that passed, resemblance scores changed by `r round(rehab_mod_fe$estimate[2], 2)` percent (remember resemblance is measured on a percentage scale), that is, they resembled their pre-zombification state less.
* The change over time of resemblance scores was significantly affected by whether the zombie had gene therapy or was in the wait list, $\hat{\gamma}$ = `r round(rehab_mod_fe$estimate[4], 2)` [`r round(rehab_mod_fe$conf.low[4], 2)`, `r round(rehab_mod_fe$conf.high[4], 2)`], *t*(`r rehab_mod_fe$df[4]`) = `r round(rehab_mod_fe$statistic[4], 2)`, *p* < .001. The rate of change in resemblance scores is `r round(rehab_mod_fe$estimate[4], 2)` higher in the intervention group than in the wait list group.

###  `r user_astronaut()` Breaking down the interaction [(3)]{.alt}

The interaction effect is the one that tests our hypothesis, but what does it mean that the rate of change in resemblance scores is `r round(rehab_mod_fe$estimate[4], 2)` higher in the intervention group than in the wait list group?

To break this interaction effect down it would be useful to know what the effect of time was separately for the gene therapy and wait list groups. In other words, get an estimate for the rate of change of resemblance scores in the gene therapy and intervention groups. We can do this using the `emtrends` function from the `emmeans` package [@lenth_emmeans_2020], which takes the general form

```{r, eval = F}
emmeans::emtrends(my_model, spec = "categorical_predictor", var = "continuous_predictor") %>% 
  tibble::as_tibble()
```
...
```{r, eval = F}
rehab_growth_simple <- emmeans::emtrends(rehab_mod, spec = "intervention", var = "time_num") %>% 
  tibble::as_tibble()
```
...
time_wl <- rehab_growth_simple$time_num.trend[1]
time_gt <- rehab_growth_simple$time_num.trend[2]
```

The output shows us that

* In the wait list group the parameter for time is `r round(time_wl, 2)` suggesting that for every month that passes resemblance scores change by `r round(time_wl, 2)`, that is they decrease: the zombies look *less* like their pre-zombified state. They look worse.
* In the gene therapy group the parameter for time is `r round(time_gt, 2)` suggesting that for every month that passes resemblance scores change by `r round(time_gt, 2)`, that is they increase: the zombies look *more* like their pre-zombified state. They look better.
* If we take the difference between these parameter values we get the parameter for the interaction term (look back to the previous section), which was `r round(rehab_mod_fe$estimate[4], 2)`: $\hat{\gamma}_\text{time (gene therapy)} - \hat{\gamma}_\text{time (wait list)}$ = `r round(time_gt, 2)` - (`r round(time_wl, 2)`) = `r round(time_gt, 2)` + `r abs(round(time_wl, 2))` = `r round(time_gt, 2) - round(time_wl, 2)`.
...
* The standard deviation of the intercepts was `r round(rehab_mod_re$estimate[1], 2)`, 95% CI [`r round(rehab_mod_re$conf.low[1], 2)`, `r round(rehab_mod_re$conf.high[1], 2)`]. This is the standard deviation of the values of the intercepts across the zomnbies. In other words, it quantifies how variable the baseline resemblance scores are around the average baseline resemblance score (i.e. around the overall estimate of `r round(rehab_mod_fe$estimate[1], 2)`).
* The standard deviation of the rate of change of resemblance scores  was `r round(rehab_mod_re$estimate[3], 2)`, 95% CI [`r round(rehab_mod_re$conf.low[3], 2)`, `r round(rehab_mod_re$conf.high[3], 2)`]. This is the standard deviation of the slopes (the rate of change of resemblance over time) across the zombies. In other words, it quantifies how variable the individual change in resemblance over time (within zombies) are around the group-level change over time (`r round(rehab_mod_fe$estimate[3], 2)`).
* The correlation between slopes and intercepts across the zombies was `r round(rehab_mod_re$estimate[2], 2)`, 95% CI [`r round(rehab_mod_re$conf.low[2], 2)`, `r round(rehab_mod_re$conf.high[2], 2)`]
...
```{r, echo = F}
rehab_quad_fe <- broom.mixed::tidy(rehab_quad_rs, conf.int = T, effects = "fixed")
rehab_quad_re <- broom.mixed::tidy(rehab_quad_rs, conf.int = T, effects = "ran_pars")
quad_p <- round(rehab_quad_fe$`p.value`[6], 3)
```

Focussing on the interaction term (because that test our hypothesis), the fixed effects show that

* The linear change over time of resemblance scores was significantly affected by whether the zombie had gene therapy or was in the wait list, $\hat{\gamma}$ = `r round(rehab_quad_fe$estimate[5], 2)` [`r round(rehab_quad_fe$conf.low[5], 2)`, `r round(rehab_quad_fe$conf.high[5], 2)`], *t*(`r rehab_quad_fe$df[5]`) = `r round(rehab_quad_fe$statistic[5], 2)`, *p* < .001. The linear rate of change in resemblance scores is significantly different in the intervention group compared to the wait list.
* The quadratic change over time of resemblance scores was significantly affected by whether the zombie had gene therapy or was in the wait list, $\hat{\gamma}$ = `r round(rehab_quad_fe$estimate[6], 2)` [`r round(rehab_quad_fe$conf.low[6], 2)`, `r round(rehab_quad_fe$conf.high[6], 2)`], *t*(`r rehab_quad_fe$df[6]`) = `r round(rehab_quad_fe$statistic[6], 2)`, *p* = `r quad_p`. The nonlinear rate of change in resemblance scores is significantly different in the intervention group compared to the wait list.

The observant among you will notice that the parameter values in the model with the quadratic trend are very different to those in the model that included the raw variable of [time_num]{.alt}. With [time_num]{.alt} as a predictor the interaction term for that variable and the intervention group was `r round(rehab_mod_fe$estimate[4], 2)` but with the quadratic term included we get `r round(rehab_quad_fe$estimate[5], 2)` for the linear trend, which is several orders of magnitude different. What's going on? The `poly()` function creates what are known as orthogonal polynomials, that means that they are independent from each other. This independence is handy because it means we can evaluate the linear and nonlinear trends independently (that is, we evaluate the specific effect of the nonlinear trend). The price we pay is that the effect of time has to be rescaled to create this independence and the parameters no longer map onto our original scale of measurement. However, a simple way to interpret the nonlinear interaction is to visualise it. 
...
* The standard deviation of the intercepts was `r round(rehab_quad_re$estimate[1], 2)`, 95% CI [`r round(rehab_quad_re$conf.low[1], 2)`, `r round(rehab_quad_re$conf.high[1], 2)`]. This is the standard deviation of the values of the intercepts across the zombies.
* The standard deviation of the linear rate of change of resemblance scores  was `r round(rehab_quad_re$estimate[4], 2)`, 95% CI [`r round(rehab_quad_re$conf.low[8], 2)`, `r round(rehab_quad_re$conf.high[8], 2)`]. This is the standard deviation of the linear slopes (the rate of change of resemblance over time) across the zombies.
* The standard deviation of the nonlinear rate of change of resemblance scores  was `r round(rehab_quad_re$estimate[6], 2)`, 95% CI [`r round(rehab_quad_re$conf.low[9], 2)`, `r round(rehab_quad_re$conf.high[9], 2)`]. This is the standard deviation of the nonlinear slopes (the rate of change of resemblance over time) across the zombies.
* The correlation between linear slopes and intercepts across the zombies was `r round(rehab_quad_re$estimate[2], 2)`, 95% CI [`r round(rehab_quad_re$conf.low[2], 2)`, `r round(rehab_quad_re$conf.high[2], 2)`].
* The correlation between nonlinear slopes and intercepts across the zombies was `r round(rehab_quad_re$estimate[3], 2)`, 95% CI [`r round(rehab_quad_re$conf.low[3], 2)`, `r round(rehab_quad_re$conf.high[3], 2)`].
* The correlation between the linear and nonlinear slopes across the zombies was `r round(rehab_quad_re$estimate[5], 2)`, 95% CI [`r round(rehab_quad_re$conf.low[10], 2)`, `r round(rehab_quad_re$conf.high[10], 2)`].
