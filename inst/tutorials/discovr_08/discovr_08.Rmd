...
It also uses these [tidyverse]{.pkg} packages [@R-tidyverse; @tidyverse2019]: [dplyr]{.pkg} [@R-dplyr], [forcats]{.pkg} [@R-forcats], [ggplot2]{.pkg} [@wickhamGgplot2ElegantGraphics2016], [readr]{.pkg} [@R-readr] and [tibble]{.pkg} [@R-tibble].
...
## `r bmu()` The linear model process [(A)]{.lbl} 
...
Its as simple as that! Like other plots we have done, we can apply a theme (I like `theme_minimal()`) in the usual way. Using the code example above add a [ggplot2]{.pkg} theme and put into place the advice in the cat-det's tip. :
...
###	`r bmu()` Extracting model information with `summary()` [(A)]{.lbl}
...
###	`r bmu()` Overall fit of the model` [(A)]{.lbl}
...
### `r user_visor()` Exploring the standard error of $\hat{b}$ [(B)]{.lbl}
...
### `r user_visor()` Confidence intervals for $\hat{b}$ [(B)]{.lbl}
...
### `r bmu()` Building the model [(A)]{.lbl}
...
### `r user_visor()` Comparing models [(B)]{.lbl}
...
### `r bmu()` Model parameter estimates ($\hat{b}$) [(A)]{.lbl}
...
The `lm()` function does not produce standardized betas but you can get them using the `model_parameters()` function from the [parameters]{.pkg} package, which takes the general form:
...
### `r user_visor()` Confidence intervals [(B)]{.lbl}
...
### `r bmu()` Significance tests [(A)]{.lbl}
...
## `r user_visor()` The beast of bias [(B)]{.lbl}
...
### `r user_visor()` Getting diagnostic plots [(B)]{.lbl}
...
### `r user_visor()` Residual plots [(B)]{.lbl}

We use residual plots (plots 1 and 3) to look for linearity and homoscedasticity. The Figure below (taken from @fielddsr22026) shows that we're looking for a random scatter of dots. Curvature in the plot indicates a lack of linearity, and a funnel shape (residuals that 'fan out') indicates heteroscedasticity. Curvature *and* a funnel shape indicates both non-linearity and heteroscedasticity. We can also use Q-Q plots to look for normality in residuals.
...
### `r user_astronaut()` Pretty residual plots [(C)]{.lbl}

With the `ggfortify` package loaded, you can use `ggplot2::autoplot()` to produce nicely formatted plots that are [ggplot2]{.pkg} objects, which means that you can add other [ggplot2]{.pkg} elements to them, such as themes.

<div class="tip">
  `r cat_space()` **Tip: Using `ggfortify`**
  
For the `autoplot()` function to work the `ggfortify` package must be loaded. (This is a situation where we can't use explicit code because `ggfortify` adds functionality to [ggplot2]{.pkg} rather than to itself.) Basically, outside of this tutorial you'd need to include `library(ggfortify)` at the start of your document for the code in this section to work.
</div>
...
You can add a [ggplot2]{.pkg} element to this plot in the usual way with the `+` symbol. For example, to apply my ever-favourite `theme_minimal()` we could use:
...
`ggfortify` also adds some useful arguments to `autoplot()` that map onto aesthetics that we have met in earlier tutorials. As with any [ggplot2]{.pkg} plot we can change colours using [hex codes](https://www.color-hex.com/). Here's some properties that we can change within `autoplot()`:
...
### `r user_visor()` Influential cases and outliers: plots [(B)]{.lbl}

As a bare minimum we should use Cook's distance to identify influential cases and standardized residuals to check for outliers. @fielddsr22026 describes a much wider battery of values that you can use to check for these things so if you're starting to get the stats bug (?!) then check that out.
...
### `r user_visor()` Influential cases and outliers: numbers [(B)]{.lbl}
...
## `r user_visor()` Unguided example [(B)]{.lbl}
...
### `r user_visor()` Diagnostics quiz [(B)]{.lbl}
...
### `r user_astronaut()` Bayesian parameter estimates [(C)]{.lbl}
