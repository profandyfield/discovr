---
title: "discovr: Comparing severeal means (GLM 1)"
author: "Andy Field"
output:
  learnr::tutorial:
    progressive: true
...
description: "Comparing several means. Essentially 'One-way independent ANOVA' but taught using a general linear model framework. Covers setting contrasts (dummy coding, contrast coding, and linear and quadratic trends. The *F*-statistic and Welch's robust *F*, robust parameter estimation, * tests, robust tests of means based on trimmed data."
...
# discovr: Comparing severeal means (GLM 1)
...
Moving onto the parameter estimates, $b_0$ (the value in the column labelled [estimate]{.alt} and the row labelled [(intercept)]{.alt}) is equal to the mean of the baseline category (the no puppies group), 2.2 (check your earlier summary statistics!). The *b*-value for the first dummy variable (labelled [dose15 mins]{.alt}) is equal to the difference between the means of the 30-minute group and the control group ($5.0 − 2.2 = 2.8$). Finally, the *b*-value for the second dummy variable (labelled [dose30 mins]{.alt}) is equal to the difference between the means of the 15-minute group and the control group ($3.2 − 2.2 = 1$). These values demonstrate how dummy coding partitions the variance in happiness scores to compare specific group means. We can see from the significance values of the associated *t*-tests that the difference between the 30-minute group and the control group is significant because *p* = 0.008, which is less than 0.05; however, the difference between the 15-minute and the control group is not (*p* = 0.282).
...
quiz(caption = "Diagnostic plot quiz (level 2)",
  ...
  question("Based on the Q-Q plot, can we assume normality of the residuals?",
    ...
    correct = "Correct - The dots on the Q-Q plot seem to deviate from the line at the extremes, which indicates a non-normal distribution. Well done!",
    random_answer_order = TRUE,
    allow_retry = T
  ),
...
We can do the same for the second contrast. We know from Table 2 that the weights for contrast 2 are: 0 (no puppies), −1/2 (15-minute group) and 1/2 (30-minute group). Remembering that the first weight we enter will be for the no puppies group, we enter the value 0 as the first weight, then $-1/2$ for the 15-minute group and finally $1/3$ for the 30-minute group. Let’s call this object [short_vs_long]{.alt} to remind us that it compares the means of the shorter puppy therapy session against the mean of the longer one:
```{r, eval = F}
...
* `contr.helmert(n)`	Each category (except the last) is compared to the mean effect of all subsequent categories. For the puppy data this would give give us a contrast comparing no puppies to the combined effect of 15- and 30-minutes, then a contrast comparing 15-minutes to 30-minutes. In fact, it would mimic our manual contrast coding.
