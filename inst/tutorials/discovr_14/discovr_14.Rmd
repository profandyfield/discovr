...
description: "Multilevel models. This tutorial looks at fitting multilevel models using the `lme4` package. It begins with an optional section on checking and coding categorical variables before moving on to show you how to fit and interpret a multilevel model. We also briefly look at the [purrr]{.pkg} package."
...
It also uses these [tidyverse]{.pkg} packages [@R-tidyverse; @tidyverse2019]: [readr]{.pkg} [@R-readr], [dplyr]{.pkg} [@R-dplyr], [forcats]{.pkg} [@R-forcats],  [ggplot2]{.pkg} [@wickhamGgplot2ElegantGraphics2016], [purrr]{.pkg} [@R-purrr], [tibble]{.pkg} [@R-tibble], and [tidyr]{.pkg} [@R-tidyr].
...
### `r bmu()` The `factor()` and `as_factor()` functions [(A)]{.lbl}

First, let's explore the potential confusion created by the different behaviour of the `as_factor()` and `factor()` functions. In my book I use the `as_factor()` function from [forcats]{.pkg} (see [discovr_01]{.alt}), but sooner or later you will come across the `factor()` function in base `r rproj()` and you might want to use it. However, the functions behave differently as we'll now demonstrate.
...
### `r bmu()` Checking the order of factor levels [(A)]{.lbl}
...
### `r bmu()` Reordering factor levels (revision) [(A)]{.lbl}

Way back in [discovr_01]{.alt}  we met the `fct_relevel()` function from the [forcats]{.pkg} package. Let's refresh our memories. The function takes this general form:
...
We might want some summary statistics of quality of life scores split by **clinic**. We can create a basic table of descriptive statistics using by grouping the data by **clinic** and then passing it into the `describe_distribution()` function of the [datawizard]{.pkg} package.
...
## `r user_visor()` Fitting fixed effect models [(B)]{.lbl}
...
To fit the fixed-effects model within each clinic separately we could filter the data to include only a single clinic, fit the model, and repeat for the other 20 clinics. There is a more elegant way to achieve this using the `map()` function from the [purrr]{.pkg} package, which enables you to apply a function to multiple items simultaneously.
...
All of these questions are difficult to answer when scanning a 100-row table. Let's make life a bit easier by plotting the information instead. Use what you know about [ggplot2]{.pkg} to plot the parameter estimates in [models]{.alt} (in the variable **estimate**) using `geom_density()` and then `facet_wrap()` to create different plots for the different predictors in the model (in the variable **term**).
...
## `r user_visor()` Fitting multilevel models [(B)]{.lbl}
###  `r user_visor()` (Attempt to) fit the model  [(B)]{.lbl}
...
###  `r user_astronaut()` A visit from the cockroach of convergence [(C)]{.lbl}
...
###  `r user_visor()` Rescaling variables  [(B)]{.lbl}
...
###  `r user_visor()` The cockroach of convergence returns  [(B)]{.lbl}
...
###  `r user_visor()` Fit the model again  [(B)]{.lbl}
...
###  `r user_visor()` Interpreting the model [(B)]{.lbl}
...
* The `estimate_slopes()` function from the [modelbased]{.pkg} package, which we met in `discover_11` and `discovr_12`.
